...
class DashaCompatibilitySection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
          ...
          child: Column(
            ...
            children: [
              ...
              if (d.comparison.explanation.isNotEmpty) ...[
                ...
                Text(d.comparison.explanation, style: body(13, color: context.onBrightness(light: AppColors.textMuted, dark: AppColors.darkTextMuted), height: 1.4)),
              ],
        ...
        _partnerSection(context, 'Bride', d.bride),
        const SizedBox(height: 10),
        _partnerSection(context, 'Groom', d.groom),
      ],
    );
  }

  Widget _partnerSection(BuildContext context, String who, PartnerDashaResult p) {
    return CompatibilityExpandableSection(
      ...
      children: [
        if (p.currentMahadasha != null)
          _periodRow(context, 'Current Mahadasha', p.currentMahadasha!.lord, p.currentMahadasha!.startDate, p.currentMahadasha!.endDate),
        if (p.currentAntardasha != null)
          _periodRow(context, 'Current Antardasha', p.currentAntardasha!.lord, p.currentAntardasha!.startDate, p.currentAntardasha!.endDate),
        if (p.nextMahadasha != null)
          _periodRow(context, 'Next Mahadasha', p.nextMahadasha!.lord, p.nextMahadasha!.startDate, p.nextMahadasha!.endDate),
        if (p.nextAntardasha != null)
          _periodRow(context, 'Next Antardasha', p.nextAntardasha!.lord, p.nextAntardasha!.startDate, p.nextAntardasha!.endDate),
        ...
        if (p.mahadashas.isNotEmpty) ...[
          ...
          CompatibilityExpandableSection(
            ...
            children: [for (final m in p.mahadashas) _timelineRow(context, m.lord, m.startDate, m.endDate)],
          ),
        ],
      ],
    );
  ...
  Widget _periodRow(BuildContext context, String label, String lord, DateTime? start, DateTime? end) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Expanded(
            child: Text(label, style: body(13, weight: FontWeight.w700, color: context.onBrightness(light: AppColors.ink, dark: AppColors.darkText))),
          ),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.end,
              children: [
                Text(humanizeCode(lord), style: body(13, weight: FontWeight.w700, color: context.onBrightness(light: AppColors.forest700, dark: AppColors.forest300))),
  ...
  Widget _timelineRow(BuildContext context, String lord, DateTime? start, DateTime? end) {
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 4),
      child: Row(
        children: [
          Expanded(child: Text(humanizeCode(lord), style: body(12, weight: FontWeight.w600, color: context.onBrightness(light: AppColors.ink, dark: AppColors.darkText)))),
          Text('${formatCompatDate(start)} - ${formatCompatDate(end)}',
              style: body(11, color: context.onBrightness(light: AppColors.textMuted, dark: AppColors.darkTextMuted))),
        ],
      ),
