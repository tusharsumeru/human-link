...
class DashaCompatibilitySection extends StatelessWidget {
  ...
  Widget _partnerSection(
    ...
    String who,
    PartnerDashaResult p,
  ) {
    return CompatibilityExpandableSection(
      title: who,
      subtitle: p.birthNakshatraName != null
          ? 'Birth Nakshatra: ${p.birthNakshatraName}'
          : null,
      children: [
        if (p.currentMahadasha != null)
          _periodRow(
            context,
            'Current Mahadasha',
            p.currentMahadasha!.lord,
            p.currentMahadasha!.startDate,
            p.currentMahadasha!.endDate,
          ),
        if (p.currentAntardasha != null)
          _periodRow(
            context,
            'Current Antardasha',
            p.currentAntardasha!.lord,
            p.currentAntardasha!.startDate,
            p.currentAntardasha!.endDate,
          ),
        if (p.nextMahadasha != null)
          _periodRow(
            context,
            'Next Mahadasha',
            p.nextMahadasha!.lord,
            p.nextMahadasha!.startDate,
            p.nextMahadasha!.endDate,
          ),
        if (p.nextAntardasha != null)
          _periodRow(
            context,
            'Next Antardasha',
            p.nextAntardasha!.lord,
            p.nextAntardasha!.startDate,
            p.nextAntardasha!.endDate,
          ),
          ...
          CompatibilityExpandableSection(
            ...
            children: [
              for (final m in p.mahadashas)
                _timelineRow(context, m.lord, m.startDate, m.endDate),
            ],
          ),
        ],
      ],
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${formatCompatDate(start)} - ${formatCompatDate(end)}',
                  style: body(
                    11,
                    color: context.onBrightness(
                      light: AppColors.textMuted,
  ...
  Widget _sandhiRow(DashaSandhiFinding s) {
    return FindingRow(
      title:
          '${humanizeCode(s.previousMahadashaLord)} → ${humanizeCode(s.nextMahadashaLord)}',
      visual: moduleStatusVisual(s.status),
      valueLabel: s.withinWindow != null
          ? (s.withinWindow! ? 'Within window' : 'Outside window')
          : null,
  ...
  Widget _marriageSignificatorRow(MarriageSignificatorDashaFinding f) {
    final matches = <String>[
      ...f.matchedMahadashaSignificators.map(
        (m) => 'Mahadasha: ${humanizeCode(m)}',
      ),
      ...f.matchedAntardashaSignificators.map(
        (m) => 'Antardasha: ${humanizeCode(m)}',
      ),
    ];
    return FindingRow(
